...
public class AutoEncoder extends LinearOpMode {
    ...
    void encoderSearch(DcMotor motor0, DcMotor motor1, int length, int f_or_b){
        if(f_or_b == 0) while (motor0.getCurrentPosition() < length);
        else while(motor1.getCurrentPosition() < length);
